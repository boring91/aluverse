...
export function CreatePayrollEmployee({ open, onOpenChange, itemId }: Props) {
  ...
  const createEmployeeAction = useMutation(
  ...
  const updateEmployeeAction = useMutation(
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (isUpdate && itemId) {
        await updateEmployeeAction.mutateAsync({
          id: Number(itemId),
          ...payload,
        });
        return;
      ...
      await createEmployeeAction.mutateAsync(payload);
    },
  });
  ...
  const isPending =
    ...
    createEmployeeAction.isPending ||
    updateEmployeeAction.isPending ||
    !paySchedules ||
    (isUpdate && !employee);

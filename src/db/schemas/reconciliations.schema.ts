...
export const reconciliations = pgTable(
  "reconciliations",
  {
    ...
    reconciliationGroup: transactionReconciledGroup().notNull(),
  ...
  (table) => {
    return [
      // ensure budget category is only set if reconciliation group is budget
      check(
        "budget_category_check_constraint",
        sql`${table.reconciliationGroup} <> 'budget' OR ${table.budgetCategoryId} IS NOT NULL`,
      ),

      // ensure project id is only set if reconciliation group is project
      check(
        "project_id_check_constraint",
        sql`${table.reconciliationGroup} <> 'project' OR ${table.projectId} IS NOT NULL`,
      ),

      // ensure loan id is only set if reconciliation group is loan
      check(
        "loan_id_check_constraint",
        sql`${table.reconciliationGroup} <> 'loan' OR ${table.loanId} IS NOT NULL`,
      ),
